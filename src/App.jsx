...
function reviewerAiRowToDraft(row) {
  ...
  const computed = row.category === "Numerical Ability" ? arithmeticFromConcept(row.question) : null;
  ...
  return {
    ...
    validation: {
      computedResult: computed?.value || "",
      storedCorrectAnswer: row.correct_answer,
      generatedDistractors: choices.filter((choice) => !sameNumericValue(choice, row.correct_answer)),
      passed: row.category !== "Numerical Ability" || (!!computed && sameNumericValue(row.correct_answer, computed.value) && choices.every(isNumericChoiceText))
    },
...
function extractReviewerConceptSources(importItem, limit = 10) {
  const seen = new Set();
  const sources = [];
  const pushSource = (text, origin, detected = {}) => {
    const cleaned = String(text || "").replace(/\s+/g, " ").trim();
    if (cleaned.length < 4 || cleaned.length > 420) return;
    const key = cleaned.toLowerCase();
    if (seen.has(key)) return;
    seen.add(key);
    const category = detected.category || categorizeText(`${importItem.name} ${cleaned}`);
    const topic = detected.subCategory || extractTopics(`${importItem.name} ${cleaned}`)[0] || (importItem.topics || [])[0] || "Reviewer Questions";
    sources.push({
      sourceReviewer: importItem.name,
      sourceText: cleaned,
      sourceOrigin: origin,
      category,
      topic: normalizeSubCategory(category, topic),
      concept: detectReviewerConcept(cleaned, category, topic)
    });
  };
  (importItem.extractedQuestions || []).forEach((q) => pushSource(q.question, "Extracted Question Stem", q));
  (importItem.concepts || []).forEach((concept) => pushSource(concept, "Extracted Concept"));
  (importItem.lessons || []).forEach((lesson) => pushSource(lesson, "Reviewer Lesson Text"));
  String(importItem.rawTextPreview || "").split(/\n+/).forEach((line) => {
    const cleaned = line.trim();
    if (/^\d{1,3}\s*[\.\)]/.test(cleaned) || /\b(divisible|percent|fraction|ratio|synonym|antonym|constitution|government|conclusion|analogy)\b/i.test(cleaned)) {
      pushSource(cleaned, "Raw Reviewer Text");
    }
  });
  return sources.slice(0, limit);
}

function detectReviewerConcept(text, category, topic) {
  const lower = text.toLowerCase();
  if (/divisible by\s+8/.test(lower)) return "Divisibility by 8";
  if (/divisible by\s+(\d+)/.test(lower)) return `Divisibility by ${lower.match(/divisible by\s+(\d+)/)[1]}`;
  if (/[÷/]\s*\d+|divid/.test(lower)) return "Division";
  if (/[×*]\s*\d+|multip/.test(lower)) return "Multiplication";
  if (/\bpercent|%/.test(lower)) return "Percentages";
  if (/\bfraction|denominator|numerator/.test(lower)) return "Fractions";
  if (/\bratio|proportion/.test(lower)) return "Ratios and Proportions";
  if (/\bsynonym/.test(lower)) return "Synonyms";
  if (/\bantonym|opposite/.test(lower)) return "Antonyms";
  if (/\bsubject|verb|tense|pronoun/.test(lower)) return "Grammar and Correct Usage";
  if (/\bconstitution|bill of rights/.test(lower)) return "Philippine Constitution";
  if (/\bconclusion|assumption|premise/.test(lower)) return "Logical Conclusions";
...
function arithmeticFromConcept(text) {
  const normalized = String(text || "")
    ...
    .replace(/\?/g, " ")
    .trim();
  const fractionMatch = normalized.match(/(\d+)\s*\/\s*(\d+)\s*([+\-])\s*(\d+)\s*\/\s*(\d+)/);
  if (fractionMatch) {
    const leftFraction = Number(fractionMatch[1]) / Number(fractionMatch[2]);
    const rightFraction = Number(fractionMatch[4]) / Number(fractionMatch[5]);
    const fractionOp = fractionMatch[3];
    const fractionValue = fractionOp === "+" ? leftFraction + rightFraction : leftFraction - rightFraction;
    if (Number.isFinite(fractionValue)) return { left: leftFraction, right: rightFraction, op: fractionOp, value: Number.isInteger(fractionValue) ? String(fractionValue) : String(Number(fractionValue.toFixed(2))), expression: `${fractionMatch[1]}/${fractionMatch[2]} ${fractionOp} ${fractionMatch[4]}/${fractionMatch[5]}` };
  }
  const expressionMatches = [...normalized.matchAll(/(-?\d+(?:\.\d+)?)\s*([+\-*/])\s*(-?\d+(?:\.\d+)?)(?:\s*[=?])?/g)];
  let match = expressionMatches[0] || null;
...
const NUMERICAL_BANNED_CHOICE_WORDS = /\b(scope|statement|argument|assumption|conclusion|reasoning|reader|passage|topic|concept)\b/i;

function isNumericChoiceText(choice) {
  const text = String(choice || "").trim();
  return /^[-+]?(?:PHP\s*)?(?:\d{1,3}(?:,\d{3})+|\d+)(?:\.\d+)?\s*(?:%|m|km|kg|hrs?|hours?|days?|pesos?)?$/i.test(text)
    || /^[-+]?\d+\s*\/\s*\d+$/.test(text);
}

function numericValue(choice) {
  const clean = String(choice || "").replace(/PHP/gi, "").replace(/,/g, "").replace(/%/g, "").trim();
  if (/^[-+]?\d+\s*\/\s*\d+$/.test(clean)) {
    const [a, b] = clean.split("/").map(Number);
    return b ? a / b : NaN;
  }
  return Number(clean.match(/[-+]?\d+(?:\.\d+)?/)?.[0]);
}
...
function isInvalidReviewerAssistedDraft(draft) {
  if (!draft || isInvalidReviewerAssistedDraftQuestion(draft.question)) return true;
  if (draft.category !== "Numerical Ability") return false;
  const computed = arithmeticFromConcept(draft.question);
  return !draft.answer
    || !computed
    || !sameNumericValue(draft.answer, computed.value)
    || !isNumericChoiceText(draft.answer)
    || (draft.choices || []).length < 4
    || (draft.choices || []).some((choice) => NUMERICAL_BANNED_CHOICE_WORDS.test(String(choice)) || !isNumericChoiceText(choice))
    || (draft.choices || []).filter((choice) => sameNumericValue(choice, draft.answer)).length !== 1;
}

function formatNumericChoice(value, decimals = 2) {
...
function numericDistractors(answerValue, seedText) {
  ...
  const decimals = String(answerValue).includes(".") ? 2 : 0;
  const step = Math.abs(numeric) < 2 ? 0.25 : Math.abs(numeric) < 20 ? 1 : Math.max(1, Math.round(Math.abs(numeric) * 0.05));
  const candidates = [-2, -1, 1, 2, 3, -3]
    .map((multiplier) => numeric + (step * multiplier))
    .filter((item) => Number.isFinite(item) && item >= 0 && Math.abs(item - numeric) > 0.0001);
  const unique = [...new Set(candidates.map((item) => formatNumericChoice(item, decimals)))].filter((item) => !sameNumericValue(item, answerValue));
  return unique.sort((a, b) => hashText(`${seedText}-${a}`) - hashText(`${seedText}-${b}`)).slice(0, 3);
}

function createReviewerAssistedQuestionDrafts({ importItem, existingQuestions, adminEmail, limit = 10 }) {
  const sources = extractReviewerConceptSources(importItem, limit);
  return sources.map((source, index) => {
    const id = `RAI-${Date.now()}-${hashText(`${source.sourceReviewer}-${source.sourceText}-${index}`)}`;
    const arithmetic = arithmeticFromConcept(source.sourceText);
    ...
    } else if (category === "Numerical Ability") {
      return null;
    } else {
    ...
    const question = makeQuestion({
      ...
      answer,
      explanation,
      hint: `Trace the item back to the reviewer concept: ${source.concept}.`,
      learningTip: "Approve only if the generated item accurately reflects the source reviewer concept and has believable distractors."
    });
    const qSig = questionSignature(question);
    const duplicates = existingQuestions.filter((item) => questionSignature(item) === qSig).slice(0, 3);
    const computed = category === "Numerical Ability" ? arithmeticFromConcept(question.question) : null;
    const validationPassed = category !== "Numerical Ability" || (!!computed && sameNumericValue(question.answer, computed.value) && !isInvalidReviewerAssistedDraft(question));
    return {
      ...question,
      draftId: id,
      source: "Reviewer Assisted AI",
      status: "Draft",
      sourceReviewer: source.sourceReviewer,
      sourceConcept: source.concept,
      sourceReviewerText: source.sourceText,
      sourceOrigin: source.sourceOrigin,
      generatedBy: adminEmail || "Admin",
      dateGenerated: new Date().toISOString(),
      tags: [category, topic, bankDifficulty, "reviewer-assisted", source.concept],
      validation: {
        computedResult: computed?.value || "",
        storedCorrectAnswer: question.answer,
        generatedDistractors: question.choices.filter((choice) => !sameNumericValue(choice, question.answer)),
  ...
  }).filter((draft) => draft && !isInvalidReviewerAssistedDraft(draft));
}
...
export default function App() {
  ...
  async function persistReviewerAiDrafts(drafts) {
    const validDrafts = drafts.filter((draft) => !isInvalidReviewerAssistedDraft(draft));
  ...
  async function approveAiQuestion(draft) {
    ...
    if (isInvalidReviewerAssistedDraft(draft)) {
      setAiStudioMessage("Approval blocked: this draft failed validation. Numerical drafts must have a computed answer that matches the stored correct answer and four numeric choices.");
      return;
    }
    ...
    const cloudPayload = {
      ...
      status: "Approved",
      tags: [...(approved.tags || []), approved.sourceReviewer, approved.sourceConcept].filter(Boolean),
  ...
  async function importReviewers(files) {
    ...
    try {
      ...
      setImportMessage(totalDetected ? `${totalDetected} reviewer question stems detected from ${uploadFiles.length} file(s). Use reviewer-assisted generation to create Draft-only questions for admin review.` : "No complete question blocks were detected, but readable concepts may still be available for Draft-only reviewer-assisted generation.");
      setScreen("admin");
    } finally {
      setImportBusy(false);
    }
  ...
  const DynamicReviewCenter = () => {
    ...
    return <section id="review-center" className="mt-4 rounded-[1.5rem] border border-white/10 bg-white/[.07] p-4 backdrop-blur-xl">
      ...
      {selectedCategory && !topicSelected && <div className="mt-3 grid gap-3 md:grid-cols-2 lg:grid-cols-4">
        {selectedCategory.subs.map((topic) => <button key={topic} onClick={() => { setSubCategory(topic); setLearningTab("Learn"); }} className="rounded-2xl bg-slate-900/45 p-4 text-left transition hover:bg-white/10"><b>{topic}</b><p className="mt-2 text-xs text-white/45">{baseQuestions.filter((q) => q.category === selectedCategory.name && q.subCategory === topic).length} questions available</p></button>)}
      </div>}
            ...
            {(progress.imports || []).map((item) => {
              ...
              return <div key={item.id} className="rounded-2xl bg-slate-900/45 p-4">
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2">
                  <button onClick={() => generateReviewerAssistedDrafts(item)} disabled={importBusy} className="rounded-xl bg-cyan-400/20 px-3 py-2 text-xs font-bold text-cyan-100 disabled:opacity-40">Generate 10 Drafts from Reviewer Concepts</button>
                  <Pill>Draft only</Pill>
                  {!detectedCount && <Pill>Uses concepts even without perfect question parsing</Pill>}
                </div>
              </div>;
            })}
          </div>
